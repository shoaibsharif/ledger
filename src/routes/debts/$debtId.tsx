...
import { ArrowLeft02Icon, Calendar02Icon, Note01Icon, PlusSignIcon } from "@hugeicons/core-free-icons";
    ...
    <div className="fixed inset-0 bg-canvas overflow-auto md:overflow-hidden flex flex-col md:flex-row font-sans">
      ...
      <div className="absolute top-6 left-6 z-50 mix-blend-difference text-surface">
        <Button 
          ...
          size="icon" 
          asChild 
          className="rounded-full w-14 h-14 bg-transparent border-2 border-current hover:bg-surface/20"
        >
          <Link to="/debts">
            <HugeiconsIcon icon={ArrowLeft02Icon} strokeWidth={2.5} className="w-6 h-6" />
          </Link>
        </Button>
      </div>
      ...
      <div className={cn(
        "relative w-full md:w-[50%] h-[50vh] md:h-full flex flex-col justify-center items-center p-8 transition-colors duration-500",
        themeClass
      )}>
        <div className="flex flex-col items-center gap-2 max-w-xl text-center">
            ...
            <div className="relative">
                <h1 className="text-[15vw] md:text-[10vw] font-display font-black leading-none tracking-tighter">
        ...
        <div className="p-8 md:p-12 pb-4 border-b border-muted">
            <div className="flex justify-between items-start">
                ...
                {debt.status !== 'settled' && (
                    ...
                    >
                        <HugeiconsIcon icon={PlusSignIcon} strokeWidth={3} className="w-6 h-6" />
                    </Button>
                )}
            </div>
        </div>
        ...
        <div className="flex-1 overflow-auto p-8 md:p-12 space-y-8">
            {debt.payments.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-muted-foreground opacity-50">
                    ...
                    <p className="font-mono text-sm uppercase tracking-widest">No payments recorded</p>
                </div>
            ) : (
